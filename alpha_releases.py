...
def set_release():
    predicted_storage = tank_storage + forecast
    forecast_cond = predicted_storage > tank_size
    storage_cond = tank_storage > tank_size * alpha
    release_policy = forecast_cond * storage_cond
    return 1 * release_policy


forecast_horizon = 3 * 60 * 60
dt = 30
rain_dt = 600
beta = 5 / 4
manning = 0.012
...
for alpha_num, alpha in enumerate(release_alphas):
    ...
    for i in range(sim_len):
        ...
        if i % int(forecast_horizon / (2 * dt)) == 0:
            forecast = np.sum(rain_volume[:, i:min(sim_len, i + int(forecast_horizon / dt))], axis=1)
        if i <= bm.last_overflow:
            release_deg = set_release()
        else:
            release_deg = 0.0
        release_Q = tank_orifice_A * Cd * np.sqrt(2 * 9.81 * (tank_storage / tank_A)) * release_deg
